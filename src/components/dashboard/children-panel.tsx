              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-stone-500 dark:text-stone-400">{child.email}</p>
                  </div>
                  <a
                ...
                {child.enrollments.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {child.enrollments.map((e) => (
                      <span
                        key={e.id}
                        className="rounded-full bg-orange-50 dark:bg-orange-900/40 px-2.5 py-1 text-xs text-orange-700 dark:text-orange-400"
                      >
                        {e.program.name}
                ...
                {child.enrollments.length === 0 && (
                  <p className="mt-2 text-xs text-stone-400 dark:text-stone-500">No active enrollments.</p>
                )}
              </motion.div>
            ))
          )}
