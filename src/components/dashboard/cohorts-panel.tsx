    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center">
          ...
          <p className="mt-1 text-xs text-slate-400">Click <strong>New Cohort</strong> above to create one.</p>
        </div>
      ) : (
        <div className="space-y-4">
          {cohorts.map((cohort) => {
              ...
              <div key={cohort.id} className="kat-card overflow-hidden">
                ...
                <div className="flex items-start gap-4">
                  <div className="min-w-0 flex-1">
                    {cohort.programName && (
                      <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">{cohort.programName}</p>
                    )}
                    <h3 className="text-base font-semibold text-slate-800">{cohort.name}</h3>
                    <p className="mt-0.5 text-xs text-slate-400">
                      {fmtDate(cohort.startsAt)} &mdash; {fmtDate(cohort.endsAt)}
                    </p>
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[11px] font-medium ${cohort.applicationOpen ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" : "bg-slate-100 text-slate-500"}`}>
                        {cohort.applicationOpen ? "Applications open" : "Applications closed"}
                      </span>
                      <span className="inline-flex items-center rounded-full bg-blue-50 px-2.5 py-0.5 text-[11px] font-medium text-blue-700 ring-1 ring-blue-200">
                  ...
                  <div className="flex shrink-0 items-center gap-1">
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${isSettingsOpen ? "bg-slate-100 text-slate-700" : "text-slate-500 hover:bg-slate-100"}`}
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${isFellowsOpen ? "bg-slate-100 text-slate-700" : "text-slate-500 hover:bg-slate-100"}`}
                    >
                  ...
                  <div className="mt-4 border-t border-slate-100 pt-4">
                    ...
                    <div className="mt-4 flex items-center justify-between gap-4 border-t border-slate-100 pt-4">
                      <label className="flex cursor-pointer items-start gap-3">
                        ...
                        />
                        <div>
                          <p className="text-sm font-medium text-slate-700">Applications Open</p>
                          <p className="text-xs text-slate-400">Show this cohort on the public fellowship page</p>
                        </div>
                      </label>
                      <button
                        type="button"
                ...
                {isFellowsOpen && (
                  <div className="mt-4 border-t border-slate-100 pt-4">
                    <p className="mb-4 text-sm font-semibold text-slate-700">Approved Fellows &amp; Program Assignments</p>
                    ...
                    {loadingFellows[cohort.id] ? (
                      ...
                      <p className="text-sm text-slate-400">No approved fellows for this cohort yet.</p>
                    ) : (
                      <div className="space-y-3">
                        {fellowsMap[cohort.id].map((fellow) => {
                          ...
                          return (
                            <div key={fellow.userId} className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
                              <div className="mb-3">
                                <p className="text-sm font-semibold text-slate-800">{fellow.firstName} {fellow.lastName}</p>
                              ...
                              {fellow.enrollments.length > 0 ? (
                                ...
                                <p className="mb-3 text-xs italic text-slate-400">No programs assigned yet.</p>
                              )}
                              ...
                              {available.length > 0 ? (
                                ...
                                <p className="text-xs italic text-slate-400">Enrolled in all available programs.</p>
                              )}
                            </div>
                          );
                        })}
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-5">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 bg-slate-50/60 p-3.5">
              ...
              />
              <div>
                <p className="text-sm font-medium text-slate-700">Open applications immediately</p>
                <p className="text-xs text-slate-400">You can change this later from cohort settings</p>
              </div>
            </label>
