  ...
  return (
    <div className="space-y-4">
      <div className="kat-card flex items-center justify-between">
        <div>
          <Link href={`/dashboard/curriculum/${programId}`} className="text-xs text-stone-400 dark:text-stone-500 hover:text-stone-600 dark:hover:text-stone-300">← Back to Curriculum</Link>
          <h2 className="mt-0.5 [font-family:var(--font-space-grotesk)] text-xl font-bold text-stone-900 dark:text-stone-100">Curriculum Versions</h2>
      ...
      {creating && (
        <div className="kat-card space-y-3 border border-[#B2401D]/30 bg-orange-50/30 dark:bg-orange-950/20">
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-stone-600 dark:text-stone-400">Changelog (optional)</label>
      ...
      {versions.length === 0 && !creating && (
        <div className="kat-card py-12 text-center">
          <Clock className="mx-auto mb-3 h-8 w-8 text-stone-300" />
          <p className="font-medium text-stone-600 dark:text-stone-400">No versions yet.</p>
          {isSA && <p className="mt-1 text-sm text-stone-400 dark:text-stone-500">Create your first curriculum version to get started.</p>}
        </div>
      )}
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-stone-400 dark:text-stone-500">v{v.versionNumber}</span>
                  <h3 className="font-semibold text-stone-900 dark:text-stone-100">{v.label}</h3>
                  {v.isActive && (
                ...
                {v.changelog && <p className="mt-1 text-sm text-stone-500 dark:text-stone-400">{v.changelog}</p>}
