      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        {loading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="rounded-2xl border border-stone-200 bg-white p-4 dark:border-stone-700 dark:bg-stone-900 sm:p-5">
          ...
          : stats.map((stat, i) => {
              const inner = (
                <div className="rounded-2xl border border-stone-200 bg-white p-4 transition hover:border-stone-300 hover:shadow-sm dark:border-stone-700 dark:bg-stone-900 dark:hover:border-stone-600 sm:p-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {actions.map((action, i) => (
              ...
              >
                <Link
                  href={action.href}
                  className="group flex items-start gap-3.5 rounded-2xl border border-stone-200 bg-white p-5 transition hover:border-stone-300 hover:shadow-sm dark:border-stone-700 dark:bg-stone-900 dark:hover:border-stone-600"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-semibold text-stone-900 dark:text-stone-100">{action.label}</p>
                      <ArrowUpRight className="size-3.5 shrink-0 text-stone-300 transition group-hover:text-stone-600" />
                    </div>
                    <p className="mt-0.5 text-xs leading-relaxed text-stone-500 dark:text-stone-400">{action.description}</p>
                  </div>
                </Link>
              </motion.div>
            ))}
