...
const STATUS_META: Record<Project["status"], { label: string; color: string; icon: React.ReactNode }> = {
  DRAFT:      { label: "Draft",        color: "bg-stone-100 text-stone-600 dark:bg-stone-800 dark:text-stone-400",           icon: <Clock className="h-3.5 w-3.5" /> },
  SUBMITTED:  { label: "Under Review", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300",            icon: <Loader2 className="h-3.5 w-3.5 animate-spin" /> },
  ...
  if (loading) {
    return (
      <div className="mt-3 space-y-2">
        <div className="h-20 animate-pulse rounded-lg bg-stone-100 dark:bg-stone-800" />
      </div>
    );
  }
    ...
    <div className="mt-3 space-y-3">
        ...
        <div className="rounded-lg border border-stone-200 bg-white dark:border-stone-800 dark:bg-stone-900">
          ...
          <div className="flex flex-wrap items-start justify-between gap-2 border-b border-stone-100 p-3 dark:border-stone-800">
            <div className="min-w-0">
              <p className="truncate font-medium text-stone-900 dark:text-stone-100">{project.title}</p>
          ...
          {project.deployedUrl && (
            <div className="border-b border-stone-100 px-3 py-2 dark:border-stone-800">
              <a
                ...
                className="flex items-center gap-1.5 text-xs text-orange-600 hover:underline dark:text-orange-400"
              >
              ...
              <div className="space-y-1">
                {project.files.map((f) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 rounded-md p-1.5 text-xs text-stone-700 transition hover:bg-stone-50 dark:text-stone-300 dark:hover:bg-stone-800"
                  >
              ...
              {uploading ? (
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 rounded-lg border border-dashed border-stone-300 px-3 py-1.5 text-xs text-stone-500 transition hover:border-orange-400 hover:text-orange-600 dark:border-stone-600 dark:hover:border-orange-500 dark:hover:text-orange-400"
                  >
                    <Upload className="h-3.5 w-3.5" /> Attach file
                  </button>
                  <button
                    onClick={() => folderInputRef.current?.click()}
                    className="flex items-center gap-1.5 rounded-lg border border-dashed border-stone-300 px-3 py-1.5 text-xs text-stone-500 transition hover:border-emerald-400 hover:text-emerald-600 dark:border-stone-600 dark:hover:border-emerald-500 dark:hover:text-emerald-400"
                  >
                    <FolderOpen className="h-3.5 w-3.5" /> Upload folder
                  </button>
                  <button
                    onClick={() => fileInputRef.current?.click()}
                    className="flex items-center gap-1.5 rounded-lg border border-dashed border-stone-300 px-3 py-1.5 text-xs text-stone-500 transition hover:border-stone-400 hover:text-stone-700 dark:border-stone-600 dark:hover:text-stone-300"
                  >
              ...
              <div className="space-y-2">
                {project.feedback.map((fb) => (
                  ...
                  >
                    ...
                    <p className="text-sm text-stone-700 dark:text-stone-300">{fb.body}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {project.status === "SUBMITTED" && (
            <div className="flex items-center gap-2 px-3 py-2 text-xs text-orange-600 dark:text-orange-400">
      ...
      {!project && !formOpen && (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-lg border-2 border-dashed border-stone-300 py-4 text-sm font-medium text-stone-500 transition hover:border-orange-400 hover:text-orange-600 dark:border-stone-600 dark:hover:border-orange-500 dark:hover:text-orange-400"
        >
