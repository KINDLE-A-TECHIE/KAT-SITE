...
export async function sendEmail(options: SendEmailOptions): Promise<boolean> {
  ...
  const from =
    ...
    `KAT Academy <${process.env.SMTP_USER ?? "noreply@kat.africa"}>`;
        ...
        <table width="560" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:16px;border:1px solid #e2e8f0;overflow:hidden;max-width:100%;">
          ...
          <tr>
            <td style="background:linear-gradient(135deg,#0D1F45 0%,#132B5E 45%,#1E5FAF 100%);padding:28px 32px;">
              <p style="margin:0;color:#93c5fd;font-size:11px;font-weight:600;letter-spacing:0.15em;text-transform:uppercase;">KAT Academy</p>
              <h1 style="margin:6px 0 0;color:#ffffff;font-size:22px;font-weight:700;">kat.africa</h1>
            </td>
          </tr>
          <!-- Body -->
          <tr>
          ...
          <tr>
            <td style="border-top:1px solid #f1f5f9;padding:20px 32px;background:#f8fafc;">
              <p style="margin:0;font-size:12px;color:#94a3b8;text-align:center;">
                © ${new Date().getFullYear()} KAT Academy · kat.africa<br/>
...
export function buildParentDigestEmail(opts: {
  ...
  const html = emailWrapper(`
    ...
    <p style="margin:0 0 20px;font-size:15px;color:#475569;line-height:1.6;">
      Hi ${opts.parentFirstName}, here is a summary of your child's academic activity at KAT Academy for <strong>${opts.month}</strong>.
    </p>
    ${childSections}
  ...
  const text =
    ...
    `Here is a summary of your child's activity at KAT Academy.\n\n` +
    opts.children
      .map((child) => {
      ...
      .join("\n\n") +
    `\n\nView full grades: ${dashboardUrl}\n\n© ${new Date().getFullYear()} KAT Academy`;
...
export function buildFellowApprovalEmail(opts: {
  ...
  const html = emailWrapper(`
    ...
    <p style="margin:0 0 16px;color:#475569;font-size:15px;line-height:1.6;">
      Your application to join the <strong>KAT Academy Fellowship Programme</strong> has been
...
Your KAT Academy fellowship application has been approved. Your account has been upgraded to Fellow.
...
export function buildFellowRejectionEmail(opts: {
  ...
  const html = emailWrapper(`
    ...
    <p style="margin:0 0 16px;color:#475569;font-size:15px;line-height:1.6;">
      Hi ${opts.firstName}, thank you for applying to the KAT Academy Fellowship Programme.
...
Thank you for applying to the KAT Academy Fellowship. After review, we were unable to approve your application at this time.
